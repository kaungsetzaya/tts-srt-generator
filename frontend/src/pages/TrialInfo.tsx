...
export default function TrialInfo() {
  const [lang, setLang] = useState<Lang>(
    () => (localStorage.getItem("lumix_lang") as Lang) || "mm"
  );
  const t = T[lang];
  ...
  const cardBorder = isDark ? "rgba(37,99,235,0.2)" : "rgba(37,99,235,0.15)";
  const textColor = isDark ? "#E2E8F0" : "#0F172A";
  const subtextColor = isDark ? "rgba(148,163,184,0.6)" : "#64748B";
  ...
  const features: FeatureCard[] =
    isTrial && trialUsage && trialLimits
      ? [
          {
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className="flex items-center justify-between px-4 sm:px-6 py-4 border-b backdrop-blur-xl"
        style={{
      ...
      >
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs font-bold transition-colors hover:opacity-80"
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 sm:py-8">
        {/* Trial Status Banner */}
        <div
          className="p-4 sm:p-5 rounded-2xl border backdrop-blur-xl mb-6"
          style={{
            background: cardBg,
            borderColor: isTrial
              ? "rgba(251,191,36,0.3)"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p
                  ...
                  style={{ color: isTrial ? "#fbbf24" : "#ef4444" }}
                >
                ...
                {isTrial && daysLeft > 0 && (
                  <p
                    ...
                    style={{ color: daysLeft > 3 ? "#4ade80" : "#ef4444" }}
                  >
                    {daysLeft} {t.daysLeft}
                  </p>
                )}
                {isTrial && daysLeft === 0 && (
                  <p className="text-xs font-bold mt-1 text-red-400">
                    {t.expired}
                  </p>
                )}
              </div>
        ...
        {isTrial && features.length > 0 ? (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div
                        className="flex justify-between mt-1.5 text-xs font-semibold"
                        style={{ color: subtextColor }}
                      >
                        <span>
                          {feat.used} {t.used}
            ...
            {trialLimits && (
              ...
              >
                ...
                <div className="space-y-2 text-xs">
                  {[
                    {
                      label: t.limits.charLimitStd,
                      value: `${trialLimits.charLimitStandard.toLocaleString()} chars`,
                    },
                    {
                      label: t.limits.charLimitChar,
                      value: `${trialLimits.charLimitCharacter.toLocaleString()} chars`,
                    },
                    {
                      label: t.limits.maxVideoSize,
                      value: `${trialLimits.maxVideoSizeMB}MB`,
                    },
                    {
                      label: t.limits.maxVideoDuration,
                      value: `${Math.floor(trialLimits.maxVideoDurationSec / 60)}:${String(trialLimits.maxVideoDurationSec % 60).padStart(2, "0")}`,
                    },
                    {
                      label: t.limits.aiVideoStdDuration,
                      value: `${Math.floor(trialLimits.maxAiVideoDurationSecStd / 60)}:${String(trialLimits.maxAiVideoDurationSecStd % 60).padStart(2, "0")}`,
                    },
                    {
                      label: t.limits.aiVideoCharDuration,
                      value: `${Math.floor(trialLimits.maxAiVideoDurationSecChar / 60)}:${String(trialLimits.maxAiVideoDurationSecChar % 60).padStart(2, "0")}`,
                    },
                  ].map((item, i) => (
                    <div
                      key={i}
          ...
          <div className="text-center py-16">
            ...
            <a
              href="https://t.me/LumixOfficialBot"
              target="_blank"
              ...
              style={{
                background: `linear-gradient(135deg, ${accent}, #F4B34F)`,
              }}
            >
              Telegram Bot
            </a>
