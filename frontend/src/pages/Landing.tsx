...
const C = {
  ...
  accent4: "#EF4444", // Red
  gradient1: "linear-gradient(135deg, #3B82F6 0%, #10B981 100%)",
...
const features = [
  { icon: "🎙️", title: "Voice Generation", badge: "Multi Voice", desc: "စာသားမှ Studio-Quality အသံဖိုင်များသို့ ပြောင်းလဲပေးသည်။ Speed, Pitch စိတ်ကြိုက်ချိန်ညှိနိုင်သည်။", detail: "Male & Female · Speed · Pitch · MP3/WAV", color: C.accent1 },
  { icon: "📝", title: "Smart Subtitles", badge: "Auto Timing", desc: "Video Format မည်သည့်အမျိုးအစားဖြစ်ဖြစ် SRT Subtitle ဖိုင်များကို အလိုအလျောက် ထုတ်ပေးသည်။", detail: "Auto SRT · Multi-platform · Timestamps", color: C.accent2 },
  { icon: "🎬", title: "Video Maker", badge: "One-Click", desc: "ဗီဒီယိုတင်ပြီး မြန်မာအသံထည့်လိုက်ပါ။ ဘာသာပြန်၊ အသံ၊ စာတန်းထိုး အကုန်အချောသတ်ပေးသည်။", detail: "Translate · Dub · Subtitle · Export", color: C.accent3 },
];
const steps = [
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden" style={{ background: C.bg, color: C.cream }}>

      {/* Colorful gradient blobs */}
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-[10%] left-[10%] w-[600px] h-[600px] rounded-full blur-[180px] opacity-30" style={{ background: C.accent1 }} />
        <div className="absolute top-[40%] right-[5%] w-[500px] h-[500px] rounded-full blur-[150px] opacity-25" style={{ background: C.accent2 }} />
        <div className="absolute bottom-[10%] left-[30%] w-[400px] h-[400px] rounded-full blur-[120px] opacity-20" style={{ background: C.accent3 }} />
        <div className="absolute top-[60%] left-[60%] w-[350px] h-[350px] rounded-full blur-[100px] opacity-15" style={{ background: C.accent4 }} />
      </div>

      {/* Grid pattern */}
      <div className="fixed inset-0 pointer-events-none z-0 opacity-[0.03]" style={{ backgroundImage: `linear-gradient(${C.cream} 1px, transparent 1px), linear-gradient(90deg, ${C.cream} 1px, transparent 1px)`, backgroundSize: "80px 80px" }} />

      {/* NAV */}
      <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-2xl flex items-center gap-6" style={{ background: C.glass, backdropFilter: "blur(24px)", border: `1px solid ${C.glassB}`, boxShadow: "0 8px 32px rgba(0,0,0,0.1)" }}>
        <div className="flex items-center gap-8">
          <span className="text-2xl font-black tracking-tight" style={{ color: C.gold }}>LUMIX</span>
          <button onClick={() => featRef.current?.scrollIntoView({ behavior: 'smooth' })} className="text-xs uppercase tracking-[0.2em] font-medium" style={{ color: C.nude }}>Features</button>
          <button onClick={() => nav("/plans")} className="text-xs uppercase tracking-[0.2em] font-medium" style={{ color: C.nude }}>Plans</button>
          <button onClick={go} className="px-6 py-2 rounded-full text-xs font-bold uppercase tracking-[0.15em]" style={{ background: C.gold, color: "#fff" }}>Login</button>
        </div>
      </nav>

      {/* HERO */}
      <div ref={heroRef} className="relative z-10 flex items-center justify-center min-h-screen px-4 pt-24">
        <motion.div style={{ opacity: heroOp }} className="text-center">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
            className="mb-6 inline-block px-4 py-1.5 rounded-full text-xs uppercase tracking-[0.4em] font-semibold"
          ...
          <div>
            ...
            <motion.h1 initial={{ opacity: 0, y: 40, rotateX: 15 }} animate={{ opacity: 1, y: 0, rotateX: 0 }} transition={{ duration: 0.8, delay: 0.2 }}
              className="font-black leading-[0.9] mb-8" style={{ fontSize: "clamp(52px, 14vw, 170px)", letterSpacing: "-0.03em", color: C.gold }}>
              STUDIO
            </motion.h1>
          </div>
          ...
          <motion.button initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, type: "spring", stiffness: 400, damping: 25 }}
            ...
            style={{ background: C.gold, color: "#fff", boxShadow: "0 8px 24px rgba(245,158,11,0.3)" }}>
            Ready to Use ↓
          </motion.button>
        </motion.div>
      </div>
      ...
      {/* ABOUT */}
      <section ref={featRef} className="relative z-10 px-6 md:px-12 py-24">
        <div className="max-w-3xl mx-auto">
          <F>
            <div className="p-10 rounded-3xl text-center" style={{ background: C.glass, backdropFilter: "blur(20px)", border: `1px solid ${C.glassB}`, boxShadow: "0 20px 60px rgba(0,0,0,0.08)" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {features.map((f, i) => (
              <F key={f.title} delay={i * 0.1}>
                <motion.div whileHover={{ y: -12, scale: 1.02 }} transition={{ duration: 0.4 }}
                  className="p-7 flex flex-col h-full rounded-3xl cursor-default"
                  style={{ background: C.glass, backdropFilter: "blur(20px)", border: `1px solid ${C.glassB}`, boxShadow: "0 16px 48px rgba(0,0,0,0.08)" }}>
                  <span className="text-3xl mb-4">{f.icon}</span>
                  <span className="text-xs font-bold uppercase tracking-[0.3em] px-2.5 py-1 rounded-full mb-3 w-fit" style={{ background: `${f.color}15`, color: f.color, border: `1px solid ${f.color}22` }}>{f.badge}</span>
      ...
      <section className="relative z-10 px-6 md:px-12 py-24">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5 relative">
            <div className="hidden md:block absolute top-7 left-[12%] right-[12%] h-0.5" style={{ background: `linear-gradient(90deg, transparent, ${C.gold}44, transparent)` }} />
            {steps.map((s, i) => (
              <F key={s.n} delay={i * 0.1} className="flex flex-col items-center text-center">
                <motion.div whileHover={{ scale: 1.1 }} className="w-14 h-14 rounded-2xl flex items-center justify-center text-sm font-black mb-4 relative z-10"
                  style={{ background: C.glass, backdropFilter: "blur(12px)", border: `1px solid ${C.glassB}`, color: C.gold, boxShadow: "0 8px 24px rgba(0,0,0,0.08)" }}>
                  {s.n}
                </motion.div>
      ...
      <F>
        <section className="relative z-10 px-6 md:px-12 py-20 text-center">
          <motion.div whileHover={{ y: -6 }} transition={{ duration: 0.4 }}
            ...
            style={{ background: C.glass, backdropFilter: "blur(24px)", border: `1px solid ${C.glassB}`, boxShadow: "0 24px 64px rgba(0,0,0,0.1)", color: C.cream }}>
      ...
      {/* FOOTER */}
      <footer className="relative z-10 px-6 py-12 text-center text-xs" style={{ color: C.nude, borderTop: `1px solid ${C.glassB}` }}>
        <p>© 2026 Lumix Studio. All rights reserved.</p>
      </footer>
    </div>
  );
}
