...
export default function Login() {
  ...
  const mut = trpc.auth.verify.useMutation({
    onSuccess: d => {
      window.location.href = d?.role === "admin" ? "/admin" : "/lumix";
    },
    onError: e => {
      setError(e.message);
    },
  ...
  return (
    <div
      ...
      style={{ background: C.bg, color: C.cream, perspective: "1000px" }}
    >
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(${C.cream}05 1px, transparent 1px), linear-gradient(90deg, ${C.cream}05 1px, transparent 1px)`,
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        >
          <div
            className="mb-6 p-4 rounded-2xl text-sm leading-relaxed"
            style={{
              background: "rgba(192,111,48,0.06)",
              border: `1px solid rgba(192,111,48,0.12)`,
              color: C.nude,
            }}
          >
            <p className="font-bold mb-2" style={{ color: C.gold }}>
              📱 Code ရယူပါ
            </p>
          ...
          <label
            className="block text-xs uppercase tracking-[0.3em] font-bold mb-3"
            style={{ color: C.gold }}
          >
            6-Digit Code
          </label>
          <div className="relative">
            ...
            {!code && (
              <span
                className="absolute inset-0 flex items-center justify-center text-3xl font-mono tracking-[0.5em] pointer-events-none"
                style={{ color: `${C.nude}33` }}
              >
                <GlitchPlaceholder />
              </span>
            )}
          ...
          {error && (
            <p className="mt-3 text-sm text-center" style={{ color: C.copper }}>
              {error}
            </p>
          )}
