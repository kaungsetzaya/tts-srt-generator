...
export default function VideoTranslator() {
  ...
  const handleFile = (f: File) => {
    ...
    if (!validTypes.some(t => f.type.startsWith("video/"))) {
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-background/80">
        ...
        <div className="flex items-center gap-3">
          <a
            href="/lumix"
            className="text-xs px-3 py-1 border border-border opacity-60 hover:opacity-100 transition-all uppercase tracking-wider"
          >
            TTS Page
          </a>
          {me?.role === "admin" && (
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="text-center py-6">
          ...
          <p className="text-sm opacity-50">
            Upload video and get Myanmar translation
          </p>
        ...
        <div
          ...
          className={`border-2 border-dashed p-12 text-center cursor-pointer transition-all ${
            dragOver
              ? "border-primary bg-[rgba(192,111,48,0.1)]"
              : file
          ...
          {file ? (
            ...
            <div>
              ...
              <p className="text-xs opacity-40 mt-2">
                MP4, WebM, MOV, AVI — max 25MB, 3 minutes
              </p>
            </div>
          )}
        </div>
        ...
        {file && (
          ...
          >
            {isLoading ? (
              <>
                <Loader2 className="w-5 h-5 animate-spin" /> Processing... (1-3
                minutes)
              </>
            ) : (
              <>
        ...
        {isLoading && (
          <div className="border border-border bg-card p-8 rounded-xl text-center">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <div
                className="w-3 h-3 rounded-full animate-pulse"
                style={{ background: C, animationDelay: "0.6s" }}
              />
            </div>
