...
import { formatDateForViewer, formatDateTimeForViewer } from "@/lib/dateTime";
import {
...
const FEATURE_ICONS: Record<string, any> = {
  tts: Mic, video_upload: FileVideo, video_link: FileVideo,
};
...
// ── User Detail Drawer ─────────────────────────────────────────
function UserDetailDrawer({ userId, userName, onClose }: { userId: string; userName: string; onClose: () => void }) {
  const { data, isLoading } = trpc.adminStats.getUserDetail.useQuery({ userId });
  ...
  const fmtTime = (d: any) => formatDateTimeForViewer(d, "—");
    ...
    <div className="fixed inset-0 z-50 flex justify-end" style={{ background: "rgba(0,0,0,0.7)" }} onClick={onClose}>
      <div className="relative w-full max-w-2xl h-full overflow-y-auto" style={{ background: "oklch(0.06 0.01 280)", borderLeft: `1px solid ${border}` }} onClick={e => e.stopPropagation()}>
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: border, background: "oklch(0.06 0.01 280)" }}>
          ...
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-lg transition-colors"><X className="w-5 h-5" /></button>
        </div>

        {isLoading ? (
          <div className="flex items-center justify-center h-64"><Loader2 className="w-8 h-8 animate-spin" style={{ color: C }} /></div>
        ) : !data ? (
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <StatBox label="Last 7 Days" value={data.recentGens} color="#4ade80" />
              <StatBox label="Total Characters" value={`${(data.totalChars / 1000).toFixed(1)}K`} color="#60a5fa" />
              <StatBox label="Total Audio Duration" value={fmtDuration(data.totalDurationMs)} color="#f472b6" />
            </div>

            {/* Success / Fail */}
            <div style={{ background: cardBg, borderColor: border }} className="border rounded-xl p-5">
              <p className="font-bold uppercase tracking-wider text-xs mb-4 opacity-60">Processing Status</p>
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-green-400" />
                  <span className="text-sm font-bold text-green-400">{data.statusBreakdown.success} Success</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-red-400" />
                  <span className="text-sm font-bold text-red-400">{data.statusBreakdown.fail} Failed</span>
                </div>
                <div className="ml-auto text-xs opacity-40">
                  {data.totalGens > 0 ? `${Math.round((data.statusBreakdown.success / data.totalGens) * 100)}% success rate` : "—"}
                </div>
              </div>
              {data.statusBreakdown.fail > 0 && (
                <div className="mt-4 h-2 rounded-full bg-red-500/30">
                  <div className="h-2 rounded-full bg-green-400" style={{ width: `${Math.round((data.statusBreakdown.success / data.totalGens) * 100)}%` }} />
                </div>
              )}
            </div>
            ...
            {/* Feature Usage */}
            <div style={{ background: cardBg, borderColor: border }} className="border rounded-xl p-5">
              <p className="font-bold uppercase tracking-wider text-xs mb-4 opacity-60">Feature Usage</p>
              {data.features.map(f => (
            ...
            <div style={{ background: cardBg, borderColor: border }} className="border rounded-xl p-5">
              <p className="font-bold uppercase tracking-wider text-xs mb-4 opacity-60">Active Hours (24h)</p>
            ...
            <div style={{ background: cardBg, borderColor: border }} className="border rounded-xl p-5">
              <p className="font-bold uppercase tracking-wider text-xs mb-4 opacity-60">Daily Generations (30d)</p>
              <div className="flex items-end gap-1 h-16">
                {data.daily.map(d => {
                  const h = maxDaily > 0 ? Math.round((d.count / maxDaily) * 64) : 4;
                  return (
                    <div key={d.date} title={`${d.date}: ${d.count}`} className="flex-1 rounded-t transition-all hover:opacity-80" style={{ height: `${h}px`, background: C, minHeight: "4px" }} />
                  );
                })}
              </div>
            </div>
            ...
            {/* Recent Logs */}
            <div style={{ background: cardBg, borderColor: border }} className="border rounded-xl p-5">
              <p className="font-bold uppercase tracking-wider text-xs mb-4 opacity-60">Recent Activity Logs</p>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {data.recentLogs.map(log => (
                  <div key={log.id} className="flex items-center gap-3 text-xs py-2 border-b border-white/5">
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${log.status === "fail" ? "bg-red-400" : "bg-green-400"}`} />
                    <span className="opacity-50 w-8">{FEATURE_LABELS[log.feature ?? "tts"]?.substring(0, 3)}</span>
                    <span className="font-mono opacity-60">{log.voice ?? log.character ?? "—"}</span>
                    <span className="opacity-40">{log.charCount}c</span>
                    {log.durationMs ? <span className="opacity-40">{fmtDuration(log.durationMs)}</span> : null}
                    {log.status === "fail" && <span className="text-red-400 truncate flex-1">{log.errorMsg}</span>}
                    <span className="ml-auto opacity-30 flex-shrink-0">{fmtTime(log.createdAt)}</span>
                  </div>
                ))}
                {data.recentLogs.length === 0 && <p className="text-xs opacity-40 text-center py-4">No recent activity</p>}
              </div>
            </div>
          </div>
        )}
...
export default function AdminDashboard() {
  ...
  const fmt = (d: any) => formatDateForViewer(d, "—");
  const fmtTime = (d: any) => formatDateTimeForViewer(d, "Never");
    ...
    <div className="min-h-screen text-foreground" style={{ background: "linear-gradient(135deg, #0F0C29 0%, #302B63 50%, #24243E 100%)" }}>
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between px-6 py-3 border-b backdrop-blur-xl" style={{ borderColor: border, background: "rgba(15,12,41,0.9)" }}>
        ...
        <div className="flex items-center gap-3">
          <a href="/lumix" className="text-xs px-3 py-1.5 border rounded-lg opacity-60 hover:opacity-100 transition-all uppercase tracking-wider" style={{ borderColor: border }}>App</a>
          <button onClick={() => logoutMutation.mutate()} className="flex items-center gap-1.5 text-xs px-3 py-1.5 border border-red-500/50 text-red-400 hover:bg-red-500/10 rounded-lg transition-all uppercase tracking-wider">
          ...
          <div className="space-y-5">
            ...
            <div className="border rounded-xl p-6" style={{ background: cardBg, borderColor: border }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-xs uppercase tracking-wider opacity-50 mb-3">Voices Ranked</p>
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-xs uppercase tracking-wider opacity-50 mb-3">Feature Breakdown</p>
                    {voiceStats?.features?.map((f: any) => {
                      const Icon = FEATURE_ICONS[f.feature] ?? Mic;
                      return (
                        <div key={f.feature} className="flex items-center gap-3 py-2 border-b border-white/5">
                          <Icon className="w-4 h-4 opacity-50" />
                          <span className="text-sm flex-1">{FEATURE_LABELS[f.feature] ?? f.feature}</span>
                          <span className="font-black text-sm" style={{ color: C }}>{f.count}</span>
                        </div>
                      );
                    })}
                  </div>
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    <div className="border rounded-xl p-3 text-center" style={{ borderColor: border }}>
                      <p className="text-xs opacity-40 mb-1">Total Chars</p>
                      <p className="text-lg font-black text-blue-400">{((voiceStats?.totalChars ?? 0) / 1000).toFixed(1)}K</p>
                    </div>
                    <div className="border rounded-xl p-3 text-center" style={{ borderColor: border }}>
                      <p className="text-xs opacity-40 mb-1">Audio Generated</p>
                      <p className="text-lg font-black text-pink-400">{fmtMs(voiceStats?.totalDurationMs ?? 0)}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-2 border rounded-xl p-6" style={{ background: cardBg, borderColor: border }}>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <button onClick={() => setChurnTab("inactive")} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${churnTab === "inactive" ? "border-red-500 text-red-400 bg-red-500/10" : "border-white/10 opacity-50"}`}>
                    <UserX className="w-3 h-3" /> Inactive ({churnData?.inactiveCount ?? 0})
                  </button>
                </div>
                <div className="max-h-52 overflow-y-auto space-y-1">
                  {(churnTab === "active" ? churnData?.activeUsers : churnData?.inactiveUsers)?.map((u: any) => (
                    <div key={u.id} className="flex items-center gap-3 py-2 border-b border-white/5 text-xs">
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${churnTab === "active" ? "bg-green-400" : "bg-red-400"}`} />
                      <span className="font-bold w-28 truncate">{u.name ?? "—"}</span>
                      <span className="opacity-40">@{u.username ?? "—"}</span>
                      <span className="ml-auto font-black" style={{ color: C }}>{u.totalGens} gens</span>
                      <span className="opacity-30">{u.lastActive ? fmtTime(u.lastActive) : "Never"}</span>
                    </div>
                  ))}
                  {!(churnTab === "active" ? churnData?.activeUsers : churnData?.inactiveUsers)?.length && (
                    <p className="text-xs opacity-30 text-center py-6">No {churnTab} users</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="border rounded-xl p-6" style={{ background: cardBg, borderColor: border }}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: "Memory", value: `${health?.memory?.used ?? 0} MB`, color: "text-yellow-400" },
                    { label: "Heap", value: `${health?.memory?.heap ?? 0} MB`, color: "text-yellow-400" },
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or username..."
                className="w-full pl-9 pr-4 py-2.5 border text-sm focus:outline-none rounded-xl" style={{ background: cardBg, borderColor: border }} />
            </div>
            ...
            <div className="border rounded-xl overflow-hidden" style={{ background: cardBg, borderColor: border }}>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredUsers.map((user: any) => {
                    ...
                    return (
                      <tr key={user.id} className={`border-b transition-colors hover:bg-white/5 ${isBanned ? "opacity-40" : ""}`} style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                        ...
                        <td className="p-3">
                          {user.subscription
                            ? <span className="text-xs px-2 py-1 rounded-md bg-green-500/20 text-green-400 font-bold uppercase">{user.subscription.plan}</span>
                            : <span className="text-xs px-2 py-1 rounded-md bg-red-500/20 text-red-400 font-bold">No Sub</span>}
                        </td>
                        <td className="p-3 text-center">
                          {days !== null ? <span className={`text-xs font-bold ${days <= 3 ? "text-red-400" : days <= 7 ? "text-yellow-400" : "text-green-400"}`}>{days}d</span> : "—"}
                        </td>
                        <td className="p-3 text-center"><span className="font-black text-sm" style={{ color: C }}>{genCount}</span></td>
                        <td className="p-3 text-xs opacity-50">{fmtTime(user.lastActive)}</td>
                        <td className="p-3">
                          {isBanned
                            ? <span className="text-xs px-2 py-1 rounded-md bg-red-500/20 text-red-400 font-bold">BANNED</span>
                            : <span className="text-xs px-2 py-1 rounded-md bg-green-500/20 text-green-400 font-bold">ACTIVE</span>}
                        </td>
                        <td className="p-3">
                          <div className="flex gap-1 flex-wrap">
                            <button onClick={() => { setSelectedUser(user.id); setShowSubModal(true); }}
                              className="flex items-center gap-1 text-xs px-2 py-1 border rounded-lg transition-all hover:bg-white/10"
                              style={{ borderColor: C, color: C }}><Plus className="w-3 h-3" /> Sub</button>
                            {user.subscription && (
                              <button onClick={() => { if (confirm("Cancel subscription?")) cancelSub.mutate({ userId: user.id }); }}
                                className="text-xs px-2 py-1 border border-red-500/50 text-red-400 hover:bg-red-500/20 rounded-lg transition-all">Cancel</button>
                            )}
                  ...
                  {filteredUsers.length === 0 && <tr><td colSpan={7} className="p-8 text-center opacity-30">No users found</td></tr>}
                </tbody>
              </table>
            </div>
              ...
              <div className="space-y-2">
                {adminUsers.map((user: any) => (
                  <div key={user.id} className="flex items-center justify-between py-2 border-b border-white/5">
                    <div>
                      <span className="font-bold">{user.telegramFirstName ?? "—"}</span>
                      <span className="text-xs opacity-40 ml-2">@{user.telegramUsername ?? "—"}</span>
                    </div>
                    {user.id !== me?.userId
                      ? <button onClick={() => { if (confirm("Remove admin role?")) setRole.mutate({ userId: user.id, role: "user" }); }}
                          className="text-xs px-2 py-1 border border-red-500/50 text-red-400 hover:bg-red-500/20 rounded-lg transition-all">Remove Admin</button>
        ...
        {tab === "reports" && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              <StatBox label="Failed Jobs" value={errorData?.failedGenerations?.length ?? 0} color="#f87171" />
              <StatBox label="System Errors" value={errorData?.systemLogs?.length ?? 0} color="#fb923c" />
              <StatBox label="Unresolved" value={errorData?.systemLogs?.filter((l: any) => !l.resolved).length ?? 0} color="#facc15" />
            </div>
                  ...
                  <tbody>
                    {errorData?.failedGenerations?.map((e: any) => (
                      <tr key={e.id} className="border-b border-white/5 hover:bg-white/5">
                        <td className="p-2 opacity-40 whitespace-nowrap">{fmtTime(e.createdAt)}</td>
                        <td className="p-2 font-mono opacity-60">{(e.userId ?? "anon").slice(0, 8)}…</td>
                        <td className="p-2">
            ...
            <div className="border rounded-xl p-6" style={{ background: cardBg, borderColor: border }}>
              ...
              {errorData?.systemLogs?.length === 0 ? (
                <p className="text-xs opacity-30 text-center py-6">No system error logs</p>
              ) : errorData?.systemLogs?.map((log: any) => (
                <div key={log.id} className="border rounded-lg p-4 mb-3" style={{ borderColor: log.resolved ? "rgba(74,222,128,0.2)" : "rgba(248,113,113,0.3)", background: log.resolved ? "rgba(74,222,128,0.03)" : "rgba(248,113,113,0.05)" }}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!log.resolved && (
                        <button onClick={() => resolveError.mutate({ id: log.id })}
              ...
              {selectedPlan === "trial" && (
                <div>
                  ...
                  <input type="number" min={1} max={365} value={trialDays} onChange={e => setTrialDays(Number(e.target.value))}
                    className="w-full border p-2 text-sm focus:outline-none text-center font-bold rounded-lg" style={{ background: "rgba(0,0,0,0.4)", borderColor: border, color: C }} />
                </div>
              )}
              <div>
